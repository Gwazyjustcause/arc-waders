...
const DataRepository = (() => {
  ...
  return { workshopStations, quests, skillPhases, traderMeta, mapVisuals };
})();
...
const WorkshopView = (() => {
  ...
  const renderStation = (station) => {
    ...
    const mediaAlt = station.imageAlt ?? `${station.name} visual from ARC Raiders`;
    const media = station.image
      ? `<figure class="card-media workshop-media">
          <img src="${station.image}" alt="${mediaAlt}" loading="lazy" />
        </figure>`
      : '';
    card.innerHTML = `
      ${media}
